...
Program: DFMEdit
Unit: dfmparse_u
Copyright: (C) 2006 Frank Wunderlich (FrankWu@gmx.de)
Functions for dfm-parsing

License: GPL ( http://gnu.org/liceses/gpl.html )
*}
...
uses main_u, preview_u, functions_u, rttifunctions_u,localize_u;
...
begin
  ...
  begin
    result:=result+Chr(StrToInt('$'+copy(HexValue,i*2+1,2)));
  end;
end;
...
begin
  s:=trim(src.strings[line]);
  addstring:='';
  tn_root:=TV.Items.AddChild(node,s+'>');
...
function EncodeSpecialChars(ins:string;indent:integer):string;
var s,prop,value:string;
    i,p,c,lc:integer;
begin
  p:=pos('=',ins);
  prop:=trim(copy(ins,1,p-1));
  value:=trim(copy(ins,p+1,length(ins)-p));
  p:=pos('''',value);
  lc:=0;
  ...
  if prop<>'' then
    s:=prop+' = '+s;
  result:=StringOfChar(' ',indent*2)+s;
end;
...
begin
  ...
  begin
    ...
    if TDataRec(tn.Data).Kind=dkStrings then
      //slout.add
      result:=result+(EncodeSpecialChars(''''+s+'''',tn.level+1))+#13#10
    else
      //slout.add
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        end else {slout.add}result:=result+(encodeSpecialChars(tn_prop.Text,tn_prop.level))+#13#10;
      end;
...
begin
  ...
  begin
    ...
    if (usage=usg_preview) and isObject(tn) then
    ...
    if (not ignorecontrol) and ({(tn.level>0) or }not isObject(tn) or (usage=usg_save)) then
    begin
      ...
      if propertyExists and (usage=usg_preview) and not isObject(tn) and (assigned(Comp)) and (pos('=',tn.text)>0) then
      begin
        ...
        begin
          ...
          propertyExists:=isProperty(Comp,prop) and (ignoreSl.IndexOf(lowercase(prop))=-1);
      ...
      begin
        ...
        begin
          //output.add
          result:=result+(EncodeSpecialChars(tn.text,tn.Level))+#13#10;
        end else
        begin
          temp:=lowercase(prop);
          ...
          begin
            ...
            else if ((prop<>'')and (value<>'') and isValueValid(comp,trim(copy(tn.Text,1,pos('=',tn.text) -1)),value)) then
              //output.Add
  ...
  if assigned(Comp) then
    freeandnil(comp);
  //if ignorecontrol then tn2:=tn2.Parent;
  for i:=tn2.Level-1 downto 0 do
    //output.add
